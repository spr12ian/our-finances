...
class SpreadsheetAnalyzer:
    ...
    def write_tables_py(self) -> None:
        lines = ["# Auto generated by spreadsheet_analyzer"]
        for sheet_name in self.all_sheet_names:
            class_name = to_class_name(sheet_name)
            table_name = to_table_name(sheet_name)
            lines.append(
                f"from finances.classes.table_{table_name} import {class_name}"
            )
        self.write_lines("tables.py", lines)
